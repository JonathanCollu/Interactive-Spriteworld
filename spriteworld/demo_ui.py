...
def setup_run_ui(env, env_config, render_size, task_hsv_colors, anti_aliasing):
  ...
  while episode < 10:
    ...
    if type(timestep) == tuple: timestep = timestep[0]
    obs[episode]['next_obs'].append(timestep.observation["image"]/255)
    if env.reset_next_step: reset = True
