...
def get_config(mode=None):
  ...
  distractor_factors = distribs.Product([
      ...
      distribs.Continuous('c0', 0.5, 0.9),
  ...
  target_factors = distribs.Product([
      ...
      distribs.Discrete('shape', ['square', 'triangle', 'circle']),
      distribs.Discrete('scale', [0.13]),
      distribs.Discrete('c0', [0.16]),
      distribs.Discrete('c1', [1.]),
      distribs.Discrete('c2', [1.]),
  ])

  goal_sprite_gen = sprite_generators.generate_sprites(
      goal_factors, num_sprites=1)
  ...
  distractor_sprite_gen = sprite_generators.generate_sprites(
      distractor_factors, num_sprites=2)

  sprite_gen = sprite_generators.chain_generators(goal_sprite_gen, target_sprite_gen)
  sprite_gen = sprite_generators.chain_generators(sprite_gen, distractor_sprite_gen)

  sprite_gen = sprite_generators.chain_generators(sprite_gen, agent_body_gen)

  task = tasks.FindGoalTarget(goal_sprite=0, terminate_distance=TERMINATE_DISTANCE, target=1)
